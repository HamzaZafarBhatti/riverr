        ...
        <div class="py-4 text-gray-400 dark:text-gray-400">
            <ul class="space-y-3">
                ...
                @foreach ($links as $link)
                    ...
                    @if (!$link['childs'])
                        <li
                            class="relative px-4 flex items-center group {{ $link['is_active'] ? 'py-1 bg-green-100 hover:bg-green-200 border-r-4 border-[#0C7F4D] border-solid' : '' }}">
                            <a href="{{ $link['href'] }}"
                                ...
                                {{-- Text --}}
                                <span
                                    class="ltr:ml-4 rtl:mr-4 text-[13px] font-medium {{ $link['is_active'] ? 'text-green-600 group-hover:text-green-900' : 'text-gray-600 group-hover:text-gray-900' }}">{{ $link['text'] }}</span>

                            </a>
                        </li>
                    @else
                        {{-- Dropdown --}}
                        {{-- {{ json_encode($link['is_active']) }} --}}
                        <li class="relative px-4 group " x-data="{ open: {{ json_encode($link['is_active']) }} }">
                            <button
                                ...
                                <span class="inline-flex items-center">
                                    ...
                                    {{-- Text --}}
                                    <span
                                        class="ltr:ml-4 rtl:mr-4 text-[13px] font-medium text-gray-600 group-hover:text-gray-900">{{ $link['text'] }}</span>
                            ...
                            <template x-if="open">
                                <ul x-transition:enter="transition-all ease-in-out duration-300"
                                    ...
                                    @foreach ($link['childs'] as $child)
                                        <li
                                            class="px-2 py-1 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 {{ $child['is_active'] ? 'bg-green-100 hover:bg-green-200 text-green-500 hover:text-green-700 border-r-4 border-[#0C7F4D] border-solid' : '' }}">
                ...
                <li class="relative px-4 flex items-center group !mt-8">
                    <div class="text-xs font-normal text-gray-400 tracking-wider">v{{ config('global.version') }}
                        ©{{ config('app.name') }}</div>
                </li>
                    ...
                    @if (!$link['childs'])
                        <li class="relative px-4 flex items-center group">
                            <a href="{{ $link['href'] }}"
                                ...
                                {{-- Text --}}
                                <span
                                    class="ltr:ml-4 rtl:mr-4 text-xs font-medium text-gray-600 group-hover:text-gray-900">{{ $link['text'] }}</span>
                        ...
                        <li class="relative px-4 group" x-data="{ open: false }">
                            <button
                                ...
                                <span class="inline-flex items-center">
                                    ...
                                    {{-- Text --}}
                                    <span
                                        class="ml-4 rtl:mr-4 text-xs font-medium text-gray-600 group-hover:text-gray-900">{{ $link['text'] }}</span>
