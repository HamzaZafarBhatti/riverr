<div class="w-full">
    ...
    <div class="max-w-4xl mx-auto mb-16">
        <div class="text-center">
            ...
            {{-- Icon --}}
            <div class="h-28 w-28 bg-[#DFF6F8] dark:bg-zinc-700 rounded-full flex items-center justify-center mx-auto">
                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40" fill="none">
                    <path
                        d="M33.75 11.25H8.75C8.41848 11.25 8.10054 11.1183 7.86612 10.8839C7.6317 10.6495 7.5 10.3315 7.5 10C7.5 9.66848 7.6317 9.35054 7.86612 9.11612C8.10054 8.8817 8.41848 8.75 8.75 8.75H30C30.3315 8.75 30.6495 8.6183 30.8839 8.38388C31.1183 8.14946 31.25 7.83152 31.25 7.5C31.25 7.16848 31.1183 6.85054 30.8839 6.61612C30.6495 6.3817 30.3315 6.25 30 6.25H8.75C7.75544 6.25 6.80161 6.64509 6.09835 7.34835C5.39509 8.05161 5 9.00544 5 10V30C5 30.9946 5.39509 31.9484 6.09835 32.6517C6.80161 33.3549 7.75544 33.75 8.75 33.75H33.75C34.413 33.75 35.0489 33.4866 35.5178 33.0178C35.9866 32.5489 36.25 31.913 36.25 31.25V13.75C36.25 13.087 35.9866 12.4511 35.5178 11.9822C35.0489 11.5134 34.413 11.25 33.75 11.25ZM33.75 31.25H8.75C8.41848 31.25 8.10054 31.1183 7.86612 30.8839C7.6317 30.6495 7.5 30.3315 7.5 30V13.5359C7.90137 13.6782 8.32416 13.7506 8.75 13.75H33.75V31.25ZM26.25 21.875C26.25 21.5042 26.36 21.1416 26.566 20.8333C26.772 20.525 27.0649 20.2846 27.4075 20.1427C27.7501 20.0008 28.1271 19.9637 28.4908 20.036C28.8545 20.1084 29.1886 20.287 29.4508 20.5492C29.713 20.8114 29.8916 21.1455 29.964 21.5092C30.0363 21.8729 29.9992 22.2499 29.8573 22.5925C29.7154 22.9351 29.475 23.228 29.1667 23.434C28.8584 23.64 28.4958 23.75 28.125 23.75C27.6277 23.75 27.1508 23.5525 26.7992 23.2008C26.4475 22.8492 26.25 22.3723 26.25 21.875Z"
                        fill="#649398" />
                </svg>
            </div>
                ...
                @foreach ($payment_methods as $p)
                    <label wire:key="payment-method-{{ $p->uid }}"
                        ...
                        <span class="flex items-center text-sm">
                            <input type="radio" x-model="selected_method" wire:model.defer="selected_method"
            ...
            <div class="lg:ltr:pl-10 lg:rtl:pr-10">
                ...
                <div class="w-full mb-6">
                    <div class="relative w-full">
                        <input wire:model.defer="amount" type="text" id="deposit-amount-input"
                        ...
                        <div
                            class="absolute inset-y-0 ltr:right-0 rtl:left-0 flex items-center ltr:pr-4 rtl:pl-4 font-light text-[15px] tracking-widest dark:text-gray-300 text-gray-400">
                ...
                <div class="flex items-center justify-center space-x-2 rtl:space-x-reverse mt-3">
                    <svg class="w-4 h-4 -mt-px dark:text-zinc-400" stroke="currentColor" fill="currentColor"
                        stroke-width="0" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
                        <path
            ...
            <div class="lg:ltr:pl-10 lg:rtl:pr-10">
                ...
                <section class="bg-slate-100 p-8 rounded-lg dark:bg-zinc-800 mb-10">
                    ...
                    <h2 class="font-bold text-gray-900 text-sm dark:text-white flex items-center justify-between mb-10">
                        ...
                        <div wire:click="back"
                            class="cursor-pointer font-normal text-slate-500 text-sm hover:text-slate-700">
                    ...
                    <dl class="mt-6 space-y-2">
                        ...
                        @if (
                            $payment_currency &&
                                is_array(config('money.' . $payment_currency)) &&
                                !empty($payment_gateway->exchange_rate) &&
                                $payment_gateway->exchange_rate != settings('currency')->exchange_rate)
                    ...
                    <div class="w-full">
                        <form method="post"
                            ...
                            <input id="AmountToPay" name="AmountToPay"
                                value="{{ $payment_gateway_params['cpay']['AmountToPay'] }}" type="hidden" />
                            <input id="PayToMerchant" name="PayToMerchant"
                                value="{{ $payment_gateway_params['cpay']['PayToMerchant'] }}" type="hidden" />
                            <input id="MerchantName" name="MerchantName"
                                value="{{ $payment_gateway_params['cpay']['MerchantName'] }}" type="hidden" />
                            <input id="AmountCurrency" name="AmountCurrency"
                                value="{{ $payment_gateway_params['cpay']['AmountCurrency'] }}" type="hidden" />
                            ...
                            <input id="PaymentOKURL" name="PaymentOKURL"
                                value="{{ $payment_gateway_params['cpay']['PaymentOKURL'] }}" type="hidden" />
                            <input id="PaymentFailURL" name="PaymentFailURL"
                                value="{{ $payment_gateway_params['cpay']['PaymentFailURL'] }}" type="hidden" />
                            <input id="CheckSumHeader" name="CheckSumHeader"
                                value="{{ $payment_gateway_params['cpay']['CheckSumHeader'] }}" type="hidden" />
                            ...
                            <input id="OriginalAmount" name="OriginalAmount"
                                value="{{ $payment_gateway_params['cpay']['OriginalAmount'] }}" type="hidden" />

                            <button type="submit"
                                class="bg-primary-50 border-primary-200 focus:outline-none focus:ring focus:ring-opacity-50 focus:ring-primary-500 font-semibold hover:bg-primary-100/50 inline-flex items-center justify-center leading-5 px-3 py-3.5 rounded text-primary-800 text-sm w-full">
                ...
                @if ($selected_method === 'flutterwave' && isset($payment_gateway_params['flutterwave']))
                    <div class="w-full">
                        ...
                        <script>
                            window.makeFlutterwavePayment = function() {
                                FlutterwaveCheckout({
                                    ...
                                    amount: Number(
                                        "{{ convertToNumber(number_format(($amount * payment_gateway('flutterwave')?->exchange_rate) / settings('currency')->exchange_rate, 2, '.', '')) }}"
                                        ),
                        ...
                        {{-- Pay --}}
                        <button @click="window.makeFlutterwavePayment" type="button" id="flutterwave-payment-button"
                            class="bg-primary-50 border-primary-200 focus:outline-none focus:ring focus:ring-opacity-50 focus:ring-primary-500 font-semibold hover:bg-primary-100/50 inline-flex items-center justify-center leading-5 px-3 py-3.5 rounded text-primary-800 text-sm w-full">
                            @lang('messages.t_continue')
                        </button>
                ...
                @if ($selected_method === 'paystack' && isset($payment_gateway_params['paystack']))
                    <div class="w-full">
                        ...
                        <script>
                            window.makePaystackPayment = function() {
                                let handler = PaystackPop.setup({
                                    ...
                                    email: '{{ auth()->user()->email }}',
                                    amount: Number(
                                        "{{ convertToNumber(number_format(($amount * payment_gateway('paystack')?->exchange_rate) / settings('currency')->exchange_rate, 2, '.', '')) }}"
                                        ) * 100,
                        ...
                        {{-- Pay --}}
                        <button @click="window.makePaystackPayment" type="button" id="paystack-payment-button"
                            class="bg-primary-50 border-primary-200 focus:outline-none focus:ring focus:ring-opacity-50 focus:ring-primary-500 font-semibold hover:bg-primary-100/50 inline-flex items-center justify-center leading-5 px-3 py-3.5 rounded text-primary-800 text-sm w-full">
                            @lang('messages.t_continue')
                        </button>
                ...
                @if ($selected_method === 'razorpay' && isset($payment_gateway_params['razorpay']))
                    <div class="w-full">
                        ...
                        {{-- Payment button --}}
                        <button @click="window.makeRazorpayPayment" type="button" id="razorpay-payment-button"
                            class="bg-primary-50 border-primary-200 focus:outline-none focus:ring focus:ring-opacity-50 focus:ring-primary-500 font-semibold hover:bg-primary-100/50 inline-flex items-center justify-center leading-5 px-3 py-3.5 rounded text-primary-800 text-sm w-full">
                            @lang('messages.t_continue')
                        </button>
                ...
                @if ($selected_method === 'offline' && $offline_method?->is_active)
                    <div class="w-full">
                        ...
                        {{-- Submit --}}
                        <button wire:click="offline" type="button"
                            class="bg-primary-50 border-primary-200 focus:outline-none focus:ring focus:ring-opacity-50 focus:ring-primary-500 font-semibold hover:bg-primary-100/50 inline-flex items-center justify-center leading-5 px-3 py-3.5 rounded text-primary-800 text-sm w-full">
