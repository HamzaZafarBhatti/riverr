    ...
    <div class="min-h-screen flex grow bg-slate-50 dark:bg-zinc-700">
        ...
        <div class="hidden w-full place-items-center lg:grid">
            <div class="w-full px-2 py-40 sm:py-48 sm:px-12 flex flex-col justify-center relative bg-no-repeat bg-center bg-cover h-full"
                @if (settings('auth')->wallpaper) style="background-image: url({{ src(settings('auth')->wallpaper) }})" @endif>
                <span class="absolute bg-gradient-to-b from-green-500 to-green-400 opacity-75 inset-0 z-0"></span>

                {{-- Logo --}}
                <div class="fixed top-0 ltr:left-0 rtl:right-0 hidden p-6 lg:block lg:px-12">
                    {{-- @if (settings('footer')->logo_dark) --}}
                    <a href="{{ url('/') }}" class="flex items-center">
                        <img src="{{ src(settings('footer')->logo) }}" alt="{{ settings('general')->title }}"
                            style="height: {{ settings('appearance')->sizes['header_desktop_logo_height'] }}px;">
                    </a>
                    {{-- @else
                            <a href="{{ url('/') }}" class="flex items-center">
                                <img src="{{ src(settings('footer')->logo) }}" alt="{{ settings('general')->title }}" style="height: {{ settings('appearance')->sizes['header_desktop_logo_height'] }}px;">
                            </a>
                        @endif --}}
                </div>
